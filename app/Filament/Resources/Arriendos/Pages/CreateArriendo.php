    ...
    protected function afterCreate(): void
    {
        ...
        foreach ($detalles as $index => $detalle) {
            ...
            $modelo = match (true) {
                ...
                str_contains($tipo, 'mangueras') => \App\Models\Mangueras::find($id),
                str_contains($tipo, 'llave torque') => \App\Models\LlavesTorque::find($id),
                default => null,
            };
            ...
            } elseif ($modelo instanceof \App\Models\Mangueras) {
                // Caso especial para Mangueras
                $descripcion = "{$modelo->Equipo} - ({$modelo->Observacion})";
...
$command = $soffice
    ...
    . ' --outdir '
    . escapeshellarg(dirname($outputPdf));

exec($command, $output, $result);

// Opcional para ver errores del comando
logger()->info('PDF DEBUG', [
    'command' => $command,
    'output' => $output,
    'result' => $result
]);
